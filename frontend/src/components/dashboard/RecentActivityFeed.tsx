...
// Generate a deterministic set of recent activities from mock data
...
function activityDescription(activity: Activity): string {
  ...
  switch (activity.type) {
    ...
    case "note":
      return `Note on ${leadName}: ${activity.notes?.slice(0, 50) ?? ""}`;
    case "status_change":
      ...
      <CardContent>
        <div className="relative space-y-0">
          ...
          {MOCK_ACTIVITIES.map((activity) => {
            ...
            const initials = activity.createdBy.name.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase();
            const avatarBg = avatarColor(activity.createdBy.name);

            return (
              <div key={activity.id} className="flex gap-4 pb-5 last:pb-0 relative">
                {/* Icon bubble */}
                <div
                  className="z-10 w-7 h-7 rounded-full flex items-center justify-center shrink-0 border-2 border-background"
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {activity.createdBy.name} · {timeAgo(activity.createdAt)}
                    </span>
                  </div>
                </div>
              </div>
